...
const ForwardModal = ({ message, onClose }) => {
  ...
  return (
    <div
      className="fixed inset-0 z-[200] flex items-end justify-center bg-black/80 p-0 backdrop-blur-sm sm:items-center sm:p-4 animate-in fade-in duration-300"
      onClick={(event) => {
    ...
    >
      <div className="app-panel flex max-h-[90vh] w-full flex-col overflow-hidden rounded-t-[2rem] sm:max-w-md sm:rounded-[2rem]">
        <div className="flex items-start justify-between gap-4 border-b border-white/5 p-5 sm:p-6">
          <div>
            <h2 className="text-xl font-black tracking-tight text-white">Forward message</h2>
            <p className="mt-1 text-sm text-gray-400">Choose a person to send this message to.</p>
          </div>
            ...
            {filteredUsers.length > 0 ? (
              filteredUsers.map((user) => (
                <button
                  ...
                  className="flex w-full items-center gap-4 rounded-2xl border border-transparent bg-white/[0.02] p-3 text-left transition-all hover:border-white/5 hover:bg-white/[0.04]"
                >
                  <div className="size-11 flex-shrink-0 overflow-hidden rounded-2xl border border-white/10 sm:size-12">
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="mt-1 text-xs text-gray-500">Send a copy to this person</p>
                  </div>
                  <div className="rounded-xl border border-white/5 bg-white/[0.03] p-2 text-gray-300 transition-all">
